...
2026-08-15 | UXUI review agent recommended a useRef StrictMode guard for the auto-redeem effect — deadlocked the page on a spinner: the ref survives StrictMode's simulated remount but TanStack MUTATION state does not (mutations aren't cached like queries), so the second mount never refired and never saw the result | live browser verification of the share-link flow | ref removed; double-fire is safe because re-accept is 200-idempotent by design (fix in the share-link commit)
2026-08-15 | First redeem implementation navigated from mutate()-level callbacks — StrictMode's unmount swallows per-call callbacks, so a successful/failed redeem never navigated | same live browser session | navigation driven by effects on mutation state instead
2026-08-15 | useCreateBookmark/useDeleteBookmark invalidated only ['bookmarks'] — a bookmark added from the collection-detail page never appeared in the list in front of the user (page reads a collections-scoped key) | UXUI + PM review agents, independently | both hooks also invalidate collectionKeys.all
