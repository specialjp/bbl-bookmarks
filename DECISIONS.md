...
**Traded away:** nothing server-side. A manual paste-token fallback shipped first, then was removed on user decision: with links as the only delivery artifact there is nothing to paste, so `/shared` left the nav entirely and became a pure link-landing route (bare visits redirect to /collections). Raw tokens minted before the link era die with their single use.
...
## ADR-016: Separate Docker stacks for dev and prod (accepted, 2026-08-15)

**Context:** one `docker-compose.yml` served two jobs — `up -d db` for the local-npm dev loop and `up --build` for the full built stack. User asked to separate Docker for development and production. Two defensible readings existed: dev compose = db only, or dev compose = full hot-reload stack; the user wasn't available mid-session, so the interpretation is recorded here instead of asked.
**Decision:** the default `docker-compose.yml` becomes the **dev** stack — db (5432 published), backend on stock `node:22-slim` running `nest --watch`, frontend on `node:22-alpine` running the Vite dev server on :3000 (strictPort/Auth0 intact), source bind-mounted with named-volume overlays for `node_modules` and `src/generated` (container `prisma generate` emits linux engines; the overlay stops them clobbering the host's darwin client through the bind mount). `docker compose up -d db` keeps working verbatim. **Prod** moves to `docker-compose.prod.yml` (explicit `-f`): the previous built stack plus `restart: unless-stopped`, an unpublished db port, and `name: bbl-prod` so dev and prod never share containers or the `pgdata` volume. Compose-set env wins over the bind-mounted `.env` (dotenv precedence), so the dev containers override `DATABASE_URL` cleanly; frontend public Auth0 values are set in the compose so a fresh clone runs before any `.env` copying. CI is untouched — it builds the Dockerfiles directly, not via compose.
**Traded away:** the conventional `docker-compose.override.yml` layering (two self-contained files are easier to grade than a base+override merge); `npm ci` in the dev containers (npm install no-ops on restart; lockfile fidelity still enforced in CI and the prod images).
**How the agent was steered:** direct user request; interpretation chosen autonomously per the ask-then-ADR rule with the session non-interactive.
